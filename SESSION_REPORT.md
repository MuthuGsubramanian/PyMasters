...
## Addendum — follow-up fixes (same day, user-requested)

1. **Daily Pipeline "failure" — fixed.** Root cause was not the pipeline: Task Scheduler refused runs with 0x800710E0 ("operator or administrator refused the request") because the task had `DisallowStartIfOnBatteries` + `StopIfGoingOnBatteries` and no catch-up. Any morning the laptop wasn't on AC, the run silently skipped (report gaps: 07-18…21, 07-24). The 06:30 run on 07-25 succeeded (exit 0, report written). Fix: task now allows battery starts, won't stop on battery, and `StartWhenAvailable` runs missed triggers after wake.
2. **HealthSentinel — retired.** Evidence showed it has NEVER run successfully: the script's em-dash was saved as UTF-8 without BOM, so PowerShell 5.1 parses it as ANSI → syntax error → exit 1 on every 15-min run since 2026-07-08. It also fully duplicates the UptimeWatchdog (identical three endpoints; watchdog is newer, 5-min, with escalation + opt-in self-heal). Disabling the task was Access-denied (registered elevated), so the script was replaced with an exit-0 tombstone documenting the retirement; original preserved as `health_sentinel.ps1.retired-2026-07-25`. Watchdog verified healthy (last run result 0).
3. **Duplicate floating Vaathiyaar chat in classroom — fixed + deployed (commit 4b8c221).** During a lesson THREE launchers stacked bottom-right: global VaathiyaarPanel FAB (z-40), Classroom's context-aware chat FAB (z-50), and the TTS trigger (z-50); on the selector, the voice pill overlapped the global FAB. Fix: Layout exposes `setVaathiyaarHidden` via Outlet context; Classroom hides the global panel during lesson phases (local chat carries lesson context/hints) and keeps it on the selector; TTS trigger and voice pill moved to `right-24`. Verified live: selector shows pill + global FAB side by side; lesson shows exactly one chat FAB + spaced TTS control. App-wide sweep found no other fixed-position corner collisions.
4. **Process fix per feedback:** overlay-collision checking (grep all `fixed bottom-/top-` elements per route/phase + corner-zoom screenshots) added to the standing QA checklist (memory: `feedback_qa_overlay_collisions`), so this class of bug is caught in future sweeps — this one was visible in my own Phase-4 screenshots and I missed it.
